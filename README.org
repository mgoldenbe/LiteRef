...
# **** Scenario
# For simplicity of this and the following examples, our list of citation functions will consist of only three functions: develops-ideas, resembles-ideas and applies-to-domain. So, let’s go to the notes of … this paper and say that this paper develops the ideas of … this one … here we go. In the following demos, we will see the power that this feature affords us.
...
Using =Emacs Lisp= expressions augmented by special variables allows for great flexibility in specifying the citation sub-graph. We will demonstrate this in action in the [[#Visualization][next section]], in which we will learn, in particular, how the citation sub-graph can be visualized.
...
# **** Scenario
# Suppose I would like to exlore the topic of compression of pattern databases. I define a citation subgraph by running a search starting with the first paper on the topic, which I am opening right now, and including all the papers that develop its ideas. That is, I am interested in the incoming arcs in the citation graph… I now press =C-c v= to invoke the =literef-show-graph= and visualize the selected subgraph. To get an idea of where the arcs and the labels come from. Let, us click open this paper’s notes and see how it cites this neighbor. Note that the distinct citation functions from all over the notes got assembled in the label… 
