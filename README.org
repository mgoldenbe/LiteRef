...
# **** Scenario
# Suppose we are studying a paper and would like to comment on this fragment. We could add an annotation containing our comment, but suppose we’d like to make our comment part of the notes associated with the paper. So, a simple highlighting annotation suffices… Let us now continue to work on the notes and add our comment… At last, we want to link to the annotation. We press =C-c a= to invoke the =literef-cite-pdf-annotation= command and select the annotation to link to. Now, even if we close the PDF, we can still click on the link to jump directly to the annotation…
