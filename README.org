...
There are several ways to import a new BibTeX entry into the database of =LiteRef=. We describe each way in a section.
*** By putting a /.bib/ file into the =drop/= directory.
The most direct way do import BibTeX entries into the database of =LiteRef= is to either download or copy the corresponding /.bib/ file into the =drop/= directory. In case the BibTeX entry needs to be written from scratch, the /.bib/ file can be either created in the =drop/= directory or in another location and then copied over.
The following actions are performed automatically as soon as a new /.bib/ file appears in the =drop/= directory:
- The BibTeX entries are extracted.
- The key for each BibTeX entry is computed.
- All the entries are added into the database subject to duplicate detection. In case a duplicate is detected, the user may choose to keep both entries, in which case the key of the new entry is modified to guarantee uniqueness.
...
*** NEEDS CHANGE TO DO IT WITHOUT LINK KLIPPER By using Link Klipper
...
To cater to the above scenario, =LiteRef= can handle  /.csv/ files produced by the Chrome’s Link Klipper extension and containing all the links reachable from the current page. Once such a /.csv/ file appears in the =drop/= directory, =LiteRef= extracts from it links to BibTeX entries and downloads the entries themselves. This feature is currently available only for DBLP. It is shown in action in the following [[demo]].
...
Each of these actions can be invoked directly when the cursor is located over a citation (such a citation is called /active/) or a buffer related to a paper (i.e. that corresponds to a file in the paper’s folder) is currently active (in which case paper is called /active/). The commands for these actions are: 
...
In addition, once can use the active citation/paper as a root for defining a sub-graph of the citation graph. This is described in a separate section below.
** Importing PDF of a paper
=LiteRef= takes a lazy approach to adding paper PDFs. Namely, importing a PDF is initiated when the user tries to open a PDF that has not been added. The search for the needed PDF proceeds in three stages:
a) If one or more PDF files exist in the =drop/= directory, the user is asked whether one of these PDFs is the one being sought. If the answer for one of these PDFs is positive, that PDF is attached to the entry and opened in =Emacs=. Otherwise, the next step is taken.
...
c) When the =BIB_MANUAL_SOURCE= variable of the server is set, =LiteRef= will open a the browser for searching in the specified source manually. Namely, the server stores the BibTeX entries for which manual download of a PDF has been requested. Once a new PDF appears in the =drop/= directory, =LiteRef= asks the user to match it with one of those BibTeX entries. If none of the entries is matched the PDF remains in the =drop/= directory for future use. For example it may be attached to an entry in the step a) above when the user tries to open the corresponding paper’s PDF.  
This process is shown in the following [[demo]].
** Working with PDF
...
*** Search
The command =literef-subgraph-helm-cite= (bound by default to =C-c )=) operates similarly to =org-ref-insert-link= explained above with the exception that the set of candidate papers is limited to the current citation sub-graph. The following [[demo]] shows this.
*** Export
