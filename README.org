...
# **** Scenario
# Suppose we are maintaining notes about two topics: Anytime Search and Pathfinding… Suppose also that notes about the Pattern Databases are maintained in a separate file (for example, by a student)… To produce a document containing all of the notes, we can include the other file… I invoke =org-export-dispatch= command, export into PDF… and open that PDF. The document contains all the notes including those of the student. Also, the bibliography has been inserted… The bibfile containing the entries used in this small survey has been produced as well… Now, suppose we would like to produce a document that contains only the notes on Pathfinding. We narrow the buffer to the corresponding subtree and invoke the export. Here is the result… 
...
# **** Scenario
# The list of citation functions in my configuration file looks like this… I now open the notes of a paper. My notes already indicate that the current paper cites this one. Let us use a citation function to indicate that the current paper both develops the ideas and compares the results to the results of the cited paper. I press =C-c f= to invoke the =literef-citation-function= command. This takes me to a completion mode, which uses the =company= package. I use the completion to insert the two citation functions separated by comma. Here we go…
...
The visualization is demonstrated by the following [[demo]].

# **** Scenario
# Suppose I would like to exlore the topic of compression of pattern databases. The PDF of the first paper on the topic is open right now, so that this paper is active. I press =C-c v= to invoke the =literef-show-selected-subgraph= and build a citation sub-graph with that paper as the source. For this first sub-graph, I will not limit the arcs, so my filter simply evaluates to =true=. The sub-graph is build and displayed… Let us see how the label of this arc came about. This label consists of two citation functions. Let’s look at the notes. Here is the first citation function and here is the other one… 
# Now, the central paper in this topic is certainly this paper from 2007. Suppose I would like to see only the papers that directly cite this paper and claim orthogonality to its ideas. So, I click on this paper and select the action of building a sub-graph. I am interested in the papers that cite this one, so that’s the =in=-direction, at depth 1 of the search and satisfying the =claims-orthogonality= predicate. Here we go… 
...
#+END_HTML   
*** Search
The command =literef-subgraph-helm= (bound by default to =C-c )=) operates similarly to =org-ref-insert-link= explained above (see [[#finding-the-keys-to-cite][Finding the keys to cite]]) with the exception that the set of candidate papers is limited to the current citation sub-graph. The following [[demo]] shows this.
# Suppose I’d like to make a list of papers on the topic of pattern databases in the order of increasing publication date. I build the citation sub-graph as before, only this time let’s invoke the build action using the helm interface… Now I press =C-c )= to invoke the  =literef-subgraph-helm= command, select all the papers and cite them. I order the new citation in the order of increasing publication date by pressing =C-c <down>= to invoke the =literef-sort-citation-link= command. Lastly, I split the citation to obtain the list of papers to my wanting.
...
# Suppose we maintain a survey organized by topics. We would like to create a document with all the notes regarding the topic of compressed pattern databases. I narrow the buffer to the sub-tree corresponding to the topic,… use the buffer to build a citation sub-graph and invoke the =literef-subgraph-export-dispatch= command to export the selected sub-graph… Let us view the resulting document. It contains all the knowledge about the chosen topic contained in the LiteRef database, with a section dedicated to each paper,… with section references to the relevant sections for citations… and with the bibliography. 
